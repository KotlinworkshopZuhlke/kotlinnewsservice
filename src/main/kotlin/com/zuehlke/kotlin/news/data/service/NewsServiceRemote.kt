...
class NewsServiceRemote(private val webClient: WebClient) {
    fun fetchNews(): Result<NewsFeed> {
        val result = try {
            ...
            val newsFeed = fetchNewsFeedFromWeb()

            if (newsFeed == null) {
                Result.failure(RuntimeException("No News Feed Found"))
            } else {
                Result.success(newsFeed)
            }

        } catch (error: Exception) {
            println(error.message +" "+ error.stackTraceToString())
    ...
    private fun fetchNewsFeedFromWeb() = webClient.get().httpRequest { httpRequest: ClientHttpRequest ->
